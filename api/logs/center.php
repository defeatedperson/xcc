...
// 获取前端传递的日志类型（默认'update'）
$logType = $_POST['log_type'] ?? 'update';
// 安全映射：限制仅允许两种日志类型，防止路径遍历
$logFileMap = [
    'update' => 'update.log',
    'sql' => 'log_to_sql_error.log',
...
if ($action === 'clear') {
    ...
    // 清空文件内容（写入空字符串）
    $clearResult = file_put_contents($logFilePath, '');
    if ($clearResult === false) {
        echo json_encode([
            'success' => false,
            'message' => '日志文件清理失败（无写入权限）',
            'log_path' => $logFilePath
        ]);
        exit;
    }
