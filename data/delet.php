...
try {
    ...
    // 4.4 SSL证书表（ssl-write.php对应表）
    if ($checkTableExists('ssl_certs')) {
        $stmt = $pdo->prepare("DELETE FROM ssl_certs WHERE domain = ?");
        $stmt->execute([$domain]);
        $deletedCounts['ssl_certs'] = $stmt->rowCount();
        writeDeleteLog("ssl_certs表删除{$deletedCounts['ssl_certs']}条记录", $domain);
    } else {
        $deletedCounts['ssl_certs'] = 0;
        writeDeleteLog("ssl_certs表不存在，跳过删除", $domain);
    }
